...
TEST(IntensityCorrectionTest, AppliesDistanceCompensationWithSensorPoseTranslation) {
    sycl::device device(sycl_points::sycl_utils::device_selector::default_selector_v);
    sycl_points::sycl_utils::DeviceQueue queue(device);
    ...
    // The point cloud is represented in a translated frame where sensor origin is (10, 0, 0).
    (*cpu_cloud.points)[0] = sycl_points::PointType(13.0f, 0.0f, 0.0f, 1.0f);
    (*cpu_cloud.intensities)[0] = 2.0f;

    Eigen::Isometry3f sensor_pose = Eigen::Isometry3f::Identity();
    sensor_pose.translation() = Eigen::Vector3f(10.0f, 0.0f, 0.0f);

    sycl_points::PointCloudShared shared_cloud(queue, cpu_cloud);
    sycl_points::algorithms::intensity_correction::correct_intensity(
        shared_cloud, 2.0f, 1.0f, 0.0f, 1000.0f, 1.0f, sensor_pose);

    ASSERT_TRUE(shared_cloud.has_intensity());
    // Relative distance is 3.0 from the sensor origin: I' = 2 * 3^2 = 18
    EXPECT_NEAR((*shared_cloud.intensities)[0], 18.0f, 1e-5f);
}
...
TEST(IntensityCorrectionWithNormalTest, AppliesIncidenceCompensationWithSensorPoseTranslation) {
    sycl::device device(sycl_points::sycl_utils::device_selector::default_selector_v);
    sycl_points::sycl_utils::DeviceQueue queue(device);
    ...
    sycl_points::PointCloudShared shared_cloud(queue, cpu_cloud);
    sycl_points::algorithms::intensity_correction::correct_intensity_with_normal(
        shared_cloud, 2.0f, 1.0f, 0.0f, 1000.0f, 0.1f, 1.0f, sensor_pose);

    ASSERT_TRUE(shared_cloud.has_intensity());
    // Relative distance is 3.0 and cos(theta)=1.0, so I' = 2 * 3^2 / 1 = 18
    EXPECT_NEAR((*shared_cloud.intensities)[0], 18.0f, 1e-5f);
}
