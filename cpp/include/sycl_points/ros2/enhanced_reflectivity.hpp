...
//
// ring_mean_ref_ and ring_mean_amb_ are retained across calls: rings with points are updated
// each scan; rings with no points in the current scan keep their previous mean, providing
// stable normalization for sparse scans.
//
...
public:
    ...
    EnhancedReflectivityCorrector() {
        ring_mean_ref_.fill(0.0f);
        ring_mean_amb_.fill(0.0f);
    }
    ...
    void apply(PointCloudShared& cloud, const sensor_msgs::msg::PointCloud2& msg, float clip_max = 5.0f) {
        ...
        // Update per-ring means for rings that have points in this scan
        for (size_t r = 0; r < MAX_RINGS; ++r) {
            if (ring_count[r] > 0) {
                const float cnt = static_cast<float>(ring_count[r]);
                ring_mean_ref_[r] = ring_sum_ref[r] / cnt;
                ring_mean_amb_[r] = ring_sum_amb[r] / cnt;
            }
        }
        ...
        for (size_t i = 0; i < N; ++i) {
            ...
            float ref = en_ref[i];
            float amb = en_amb[i];
            if (ring_mean_ref_[r] > 0.0f) ref /= ring_mean_ref_[r];
            if (ring_mean_amb_[r] > 0.0f) amb /= ring_mean_amb_[r];
