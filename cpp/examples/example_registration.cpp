...
int main() {
    ...
    const auto registration_pipeline =
        std::make_shared<sycl_points::algorithms::registration::RegistrationPipeline>(queue, pipeline_param);
    ...
    for (size_t i = 0; i < LOOP + WARM_UP; ++i) {
        ...
        t0 = std::chrono::high_resolution_clock::now();
        sycl_points::algorithms::covariance::estimate_async(source_neighbors, source_downsampled).wait_and_throw();
        sycl_points::algorithms::covariance::estimate_async(target_neighbors, target_downsampled).wait_and_throw();
        const auto dt_covariance =
            std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - t0)
                .count();

        t0 = std::chrono::high_resolution_clock::now();
        sycl_points::algorithms::covariance::estimate_normals_async(source_neighbors, source_downsampled)
            .wait_and_throw();
        sycl_points::algorithms::covariance::estimate_normals_async(target_neighbors, target_downsampled)
            .wait_and_throw();
