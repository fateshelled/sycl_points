...
PointCloudShared make_cloud(const sycl_utils::DeviceQueue& queue, size_t size) {
    ...
    for (size_t i = 0; i < size; ++i) {
        cloud.points->data()[i] = PointType(static_cast<float>(i), static_cast<float>(i + 1), static_cast<float>(i + 2), 1.0f);
...
TEST(RegistrationPipelineTest, RandomSamplingLimitsRegistrationInputSize) {
    ...
    auto aligner = [&](const PointCloudShared& source, const PointCloudShared&, const knn::KNNBase&, const TransformMatrix&,
                       const Registration::ExecutionOptions&) {
