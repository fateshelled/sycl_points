...
オドメトリ推定とは、車載や手持ちのLiDARから取得される点群データを用いて相対移動量を推定することです。
...
（IMUを使用したLiDARオドメトリ、LiDAR Inertial オドメトリについても実装はしていますが、試験的なものです。）

## 前処理

スキャンマッチングに不要な点群を削除することでマッチング精度を向上させたり、点群数を減らすことで前処理以降の処理時間を減らすことが目的です。

特にLiDARを搭載している車両やLiDARを持っている人の映り込みはなるべく削除することでマッチング精度が向上します。
...
SYCLではカーネルの起動コスト（1回のparallel_forの実行コスト）が比較的大きいので、ちょっとした計算をするのに1回カーネル起動するよりも、必要なタイミングで都度計算したほうがトータルの処理時間は短くなったりします。
...
合成共分散からマハラノビス距離を計算しているということは、ソース点とターゲット点が同じような共分散行列を持っている場合には拘束として働き、共分散楕円の向きが異なるようなミスマッチの場合には拘束が弱くなる、と私は理解しています。
...
明確な切り替えを行うことなく、Point-to-PointからPoint-to-DistributionまでをスムーズにつなぐことのできるロバストなICPマッチングであると考えられます。
...
回転式のLiDARから得られる点群は、1回のスキャンを行う間の物理的なLiDARの移動や旋回運動によって点群が歪みます。
...
歪み補正を行うには、点群の各点にタイムスタンプがついている必要があります。

### 縮退検出

まっすぐな廊下のような環境だと、ほぼ長方形の筒状の点群が得られるためマッチングが不安定になりがちです。このような状況を縮退が発生しているといいます。
...
MAP推定（最大事後確率推定）は、一般的に複数のセンサー情報を統合・融合するときに用いられます。

LiDAR点群のみでのオドメトリ推定を行う場合でも、1つ前のマッチング結果を用いることでMAP推定を行うことが可能です。

特に、サブマップ点群に繰り返し構造のある場所（柱が等間隔に並んでいる場合、天井の高さが同じ部屋が上下に存在する場合、など）を走行しているときに、隣のエリアに位置が飛んでしまうことを防ぐことができます。
...
専有格子地図のほうが計算コストはかかりますが、動的な物体（歩行者や走行車両など）がサブマップ点群に残りづらいため、ケースバイケースです。

ボクセルマップも専有格子地図も、スキャン点群をVoxel単位で積み重ねてサブマップ点群を構築します。

このとき、スキャン点群の各座標値や色・反射強度値、共分散行列をVoxel内で集約しています。座標値や色・反射強度値は算術平均、共分散行列は対数ユークリッド平均を用いて集約します。

### 共分散行列の集約

サブマップ点群を構築してから、近傍点探索を行い、共分散行列を再計算しても良いのですが、計算コストが無駄になってしまいます。
そのため、スキャン点群で計算した共分散行列を再利用することで、サブマップ点群の共分散行列を低コストに求めています。また、単純平均ではなく対数ユークリッド平均を用いることで、点群マッチングの精度も向上する効果がありました。

以下、対数ユークリッド平均の求め方です。

共分散行列は半正定値行列（すべての固有値が0以上）であり、LiDAR点群から求めた共分散行列はほぼ正定値行列（すべての固有値が0より大きい）です。正定値行列に対して固有値分解を行い、固有値行列の対角成分に対数logを取って、再構築することで、3x3の対数写像行列を得ることができます。

$$ log(Cov) = V log(\Lambda) V^T $$

この対数写像行列について、平均を計算することができます。

$$ log(Cov)_{mean} = \Sigma (V log(\Lambda) V^T) / N $$

この平均値は対数空間上の数値なので、指数写像を取ることで、共分散行列の空間に戻してあげます。平均行列を固有値分解し、固有値行列の対角成分に指数関数expを取り、再構築することで、共分散行列の平均（対数ユークリッド平均）を得ることができます。

$$ Cov_{mean} = exp(log(Cov)_{mean}) = V' exp(\Lambda') V'^T $$
